...
开启后，miao 会为每个节点额外监听一个独立的 SOCKS5 端口，流量直连对应节点（不经面板当前选中的 selector）。外部客户端可用 `socks5://ip:port` 使用：

```yaml
share:
  enabled: true
  listen: 127.0.0.1    # 分享端口监听地址，默认仅回环
  base_port: 12000     # 端口分配起点；分配结果按节点名持久化到 data/cache/share_ports.json
  username: ""         # 监听非回环地址时必填
  password: ""
```

**鉴权**：用户名和密码需同时填写或同时留空。默认 `listen` 为 `127.0.0.1`，此时可以不鉴权；一旦改成 `0.0.0.0` 或局域网 IP，就必须设置用户名和密码，否则保存会被拒绝——每个分享端口都是一个出口代理，无鉴权地暴露到网络上等于开放代理。
